...
*/
{
  ...
  //cout<<"Entropy_h("<<p<<") = "<<result<< std::endl;
  return result;
}

inline double Entropy_H(std::vector<int> Xi, int max_size)
/*
    calculate H(Xi)
...
*/
{
  ...
  //cout<<"Xi.size = "<<Xi.size()<< std::endl;
  double PXi_0 = 1 - PXi_1;        //P(Xi∉X)
  //cout<<"PXi_0 = "<<PXi_0<< std::endl;
  //cout<<"PXi_1 = "<<PXi_1<< std::endl;
  ...
  */
{
  ...
  auto intersection = (double)intersectionXi_Yj;
...
inline double each_left_Conditional_Entropy(std::vector<int> X_left_comm_vector,
  ...
  */
{
  ...
  double H_XiYj = 0;
  double MinH_XiYj = each_right_Conditional_Entropy(X_left_comm_vector, Y_right_comm_vector, max_size);
  for (int j = 2; j <= k; j++)
  {
...
//cout<<"MinH_XiYj"<<MinH_XiYj<< std::endl;
  return MinH_XiYj;
}

inline double Conditional_Entropy(Community left_comms, Community right_comms)
/*
...
{
  int k = left_comms.NC;
  const int max_size = left_comms.N > right_comms.N ? left_comms.N : right_comms.N;
  double sum = 0;
  double result = 0;
  for (int i = 1; i <= k; i++)
  {
  ...
  result = sum / k;
  //cout<<"Conditional_Entropy, K"<<result<<", "<<k<< std::endl;
  return result;
}
...
*/
{
  ...
  {
    std::cout << "info: one of the two communities set is empty" << std::endl;
    return 0;
  }
...
inline double calcNMI(std::string commsX_file, std::string commsY_file)
//对用户提供的计算overlapping NMI的接口
{
  //cout<<commsX_file<<' '<<commsY_file<< std::endl;
