...
inline double calcMutualInfo(Community comm1, Community comm2)
{
    ...
    && !cid2.empty())
  {
    ...
    for (int i = 1; i <= comm1.NC; i++)
    {
      ...
      for (auto itr = cnt[i].begin(); itr != cnt[i].end(); ++itr)
      {
        int j = itr->first;;
...
inline double calcNMI(std::string commFile1, std::string commFile2)
{
