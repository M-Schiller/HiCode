...
inline double each_left_Conditional_Entropy(
  std::vector<int> X_left_comm_vector,
  ...
  */
{
  ...
  double H_XiYj = 0;
  double MinH_XiYj =
    each_right_Conditional_Entropy(
      X_left_comm_vector,
      Y_right_comm_vector,
      max_size);
  for (int j = 2; j <= k; j++)
...
*/
{
  const int max_size = std::max(left_comms.N, right_comms.N);
  double sum = 0;
  double result = 0;
  for (int i = 1; i <= left_comms.NC; i++)
  {
...
inline double calcEntropy(Community comm)//communities set's Entropy
{
  double E = 0;
