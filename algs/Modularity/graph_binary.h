...
inline unsigned int Graph::getDegree(unsigned int node) {
...
inline double Graph::getNumberOfSelfLoops(unsigned int node) {
  assert(node >= 0 && node < nb_nodes);
...
inline double Graph::getWeightedDegree(unsigned int node)
{
  assert(node >= 0 && node < nb_nodes);
