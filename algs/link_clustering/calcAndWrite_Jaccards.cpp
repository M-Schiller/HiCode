...
int intersection_size(const std::set<int> &A, const std::set<int> &B)
{
  // only get number of elements, don't build a new set
  // this assumes the sets are ordered, which std::sets are!
  int num = 0;
  std::set<int>::const_iterator As = A.begin(), Bs = B.begin();
  while (As != A.end()
    && Bs != B.end())
  {
    if (*As < *Bs)
...
int main(int argc, char const *argv[])
{
  ...
  if (!argv[1])
  {
    ...
    std::cout << "usage:\n    " << argv[0] << " input.pairs output.jaccs" << std::endl;
    exit(1);
  }
  if (!argv[2])
  {
  ...
  std::set<int> *neighbors = nullptr;
  neighbors = new std::set<int>[num_nodes]; // this stores the network
  ...
  {
    for (std::set<int>::iterator i = neighbors[keystone].begin(); i != neighbors[keystone].end(); ++i) // neighbors of keystone
    {
      int n_i = *i;
      if (n_i == keystone)
        continue;

      for (std::set<int>::iterator j = neighbors[keystone].begin(); j != neighbors[keystone].end(); ++j) // neighbors of keystone
      {
        int n_j = *j;
        if (n_j == keystone
          || n_i >= n_j)
        ...
        int len_int = intersection_size(neighbors[n_i], neighbors[n_j]);    // my set intersection function
        ...
        double curr_jacc = len_int / (double)(neighbors[n_i].size() + neighbors[n_j].size() - len_int);
  ...
  outfile.open("./LCtime.txt", std::ios::out);
