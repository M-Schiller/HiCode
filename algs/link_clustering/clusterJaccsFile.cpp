...
int main(int argc, char const *argv[]) {
  ...
  int idx_i, idx_j;
  std::map< int, std::set<std::pair<int, int>> >::iterator iter_i, iter_j;
  std::set<std::pair<int, int>>::iterator iterS;
  while (jaccFile >> i0 >> i1 >> j0 >> j1 >> jacc)
  {
    if (jacc >= threshold) {
      ...
      iter_i = edge2iter[std::make_pair(i0, i1)];
      iter_j = edge2iter[std::make_pair(j0, j1)];
      if (iter_i != iter_j) {
  ...
  std::set<std::pair<int, int>>::iterator S;
  ...
  //outfile<<"the program has run :"<< span <<"s"<< std::endl;

  std::cout << "The partition density is:" << std::endl;
  std::cout << "    D = " << 2.0 * wSum / M << std::endl;
