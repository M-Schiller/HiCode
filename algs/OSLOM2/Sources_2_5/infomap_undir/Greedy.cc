...
void Greedy::move(bool &moved)
{
  ...
  std::vector<std::pair<int, double>> wNtoM(Nnode);
...
void Greedy::calibrate()
{
  ...
  std::vector<double>(Nmod).swap(mod_degree);
...
void Greedy::level(Node ***node_tmp, bool sort)
{
  ...
  std::vector<int> nodeInMod(Nnode);
  for (int i = 0; i < Nmod; i++)
  {
    (*node_tmp)[i] = new Node();
    (*node_tmp)[i]->index = i;
  ...
  std::vector<std::map<int, double>> wNtoM(Nmod);
  ...
  for (int i = 0; i < Nmod; i++)
  {
    ...
    {
      if (it_M->first != i) {
        (*node_tmp)[i]->links.emplace_back(it_M->first, it_M->second);
      }
    }
  }
