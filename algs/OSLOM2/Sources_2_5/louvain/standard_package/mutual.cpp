...
double mutual(std::deque<std::deque<int>> en, std::deque<std::deque<int>> ten)
{
...

  {
    ...
    for (int i = 0; i < ten.size(); i++)
    {
      std::sort(ten[i].begin(), ten[i].end());
      for (int j = 0; j < ten[i].size(); j++)
      {
        conta.insert(ten[i][j]);
        //ten_.insert(ten[i][j]);
  ...
  {
    N.emplace_back(std::deque<double>(en.size()));
  }
...
double H_x_given_y(std::deque<std::deque<int>> &en, std::deque<std::deque<int>> &ten, int dim)
{
  ...
  for (auto& j : en)
  {
    ...
    for (int i = 0; i < ten.size(); i++) {
      ...
      std::deque <int> s(dim);
      double I1_I2 = std::set_intersection(ten[i].begin(), ten[i].end(), j.begin(), j.end(), s.begin()) - s.begin();	// common
      double I1_02 = std::set_difference(ten[i].begin(), ten[i].end(), j.begin(), j.end(), s.begin()) - s.begin();
      double O1_I2 = std::set_difference(j.begin(), j.end(), ten[i].begin(), ten[i].end(), s.begin()) - s.begin();
...
double mutual3(std::deque<std::deque<int>> en, std::deque<std::deque<int>> ten)
{
  ...
  int dim;
  {
    ...
    for (auto& i : ten)
    {
      ...
      {
        all.insert(std::make_pair(i[j], all.size()));
        //ten_.insert(ten[i][j]);
      }
    }
