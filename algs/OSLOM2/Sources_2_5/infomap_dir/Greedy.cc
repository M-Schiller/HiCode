...
void Greedy::move(bool &moved)
{
  ...
  std::vector<int> randomOrder(Nnode);
  ...
  std::vector<unsigned int> redirect(Nnode, 0);
  std::vector<std::pair<int, std::pair<double, double>>> flowNtoM(Nnode);
  ...
  for (int k = 0; k < Nnode; k++) {
    ...
    // For all outLinks
    if (node[flip]->outLinks.empty()) //dangling node, add node to calculate flow below
    {
    ...
    {
      for (auto& outLink : node[flip]->outLinks)
      {
        int nb_M = node[outLink.first]->index;
        const double nb_flow = outLink.second;
    ...
    for (auto& inLink : node[flip]->inLinks)
    {
      int nb_M = node[inLink.first]->index;
      const double nb_flow = inLink.second;
    ...
    for (int j = 0; j < NmodLinks - 1; j++) {
      const int randPos = j + R->randInt(NmodLinks - j - 1);
      const int tmp_M = flowNtoM[j].first;
      const double tmp_outFlow = flowNtoM[j].second.first;
      const double tmp_inFlow = flowNtoM[j].second.second;
    ...
    for (int j = 0; j < NmodLinks; j++)
    {
      const int newM = flowNtoM[j].first;
      const double outFlowNewM = flowNtoM[j].second.first;
      const double inFlowNewM = flowNtoM[j].second.second;

      if (newM != oldM)
      {
        const double delta_exit = plogp(exitFlow + outFlowOldM + inFlowOldM - outFlowNewM - inFlowNewM) - exit;

        const double delta_exit_log_exit = -plogp(mod_exit[oldM]) - plogp(mod_exit[newM]) \
          + plogp(mod_exit[oldM] - node[flip]->exit + outFlowOldM + inFlowOldM) + plogp(mod_exit[newM] + node[flip]->exit - outFlowNewM - inFlowNewM);

        const double delta_size_log_size = -plogp(mod_exit[oldM] + mod_size[oldM]) - plogp(mod_exit[newM] + mod_size[newM])
        ...
        const double deltaL = delta_exit - 2.0*delta_exit_log_exit + delta_size_log_size;
...
void Greedy::initiate()
{
  ...
  for (int i = 0; i < Nnode; i++)
  {
    ...
    { // Normalize the weights
      ...
      for (auto& outLink : node[i]->outLinks)
      {
        sum += outLink.second;
      }
  ...
  for (int i = 0; i < Nnode; i++)
  {
    ...
    if (!node[i]->outLinks.empty())
    {
      for (unsigned j = 0; j < node[i]->outLinks.size(); j++)
      {
...
void Greedy::tune()
{
  ...
  for (int i = 0; i < Nnode; i++)
  {
    ...
    for (auto &[neighbor, weight] : node[i]->outLinks)
    {
      if (i_M != node[neighbor]->index)
      {
        mod_exit[i_M] += weight;
      }
    }
...
void Greedy::calibrate()
{
  std::vector<int>(Nmod).swap(mod_empty);
  Nempty = 0;

  std::vector<double>(Nmod).swap(mod_exit);
  std::vector<double>(Nmod).swap(mod_size);
  std::vector<double>(Nmod).swap(mod_danglingSize);
  std::vector<double>(Nmod).swap(mod_teleportWeight);
  std::vector<int>(Nmod).swap(mod_members);
...
void Greedy::prepare(bool sort)
{
  ...
  if (sort)
  {
    std::multimap<double, int> Msize;
    for (int i = 0; i < Nnode; i++)
    {
      if (mod_members[i] > 0)
      {
        Nmod++;
        Msize.emplace(mod_size[i], i);
      }
    }

    for (auto it = Msize.rbegin(); it != Msize.rend(); ++it)
    {
...
void Greedy::level(Node ***node_tmp, bool sort)
{
  ...
  std::vector<int> nodeInMod = std::vector<int>(Nnode);
  ...
  std::vector<std::map<int, double>> outFlowNtoM(Nmod);
  std::map<int, double>::iterator it_M;

  for (int i = 0; i < Nnode; i++)
  {
    const int i_M = nodeInMod[node[i]->index];
    ...
    for (unsigned j = 0; j < node[i]->outLinks.size(); j++)
    {
      ...
      if (nb != i)
      {
        ...
        {
          outFlowNtoM[i_M].emplace(nb_M, nb_flow);
        }
      }
    }
  }
  ...
  for (int i = 0; i < Nmod; i++)
  {
    for (it_M = outFlowNtoM[i].begin(); it_M != outFlowNtoM[i].end(); ++it_M)
    {
      if (it_M->first != i)
      {
        (*node_tmp)[i]->outLinks.emplace_back(it_M->first, it_M->second);
      }
    }
  }

  // Calculate inflow of links from different modules
  std::vector<std::map<int, double>> inFlowNtoM(Nmod);

  for (int i = 0; i < Nnode; i++)
  {
    const int i_M = nodeInMod[node[i]->index];

    for (unsigned j = 0; j < node[i]->inLinks.size(); j++)
    {
      ...
      if (nb != i)
      {
        ...
        {
          inFlowNtoM[i_M].emplace(nb_M, nb_flow);
        }
      }
    }
  }
  ...
  for (int i = 0; i < Nmod; i++)
  {
    for (it_M = inFlowNtoM[i].begin(); it_M != inFlowNtoM[i].end(); ++it_M)
    {
      if (it_M->first != i)
      {
        (*node_tmp)[i]->inLinks.emplace_back(it_M->first, it_M->second);
      }
    }
  }

  // Option to move to empty module
  std::vector<int>().swap(mod_empty);
  Nempty = 0;
  for (int i = 0; i < Nnode; i++)
  {
    delete node[i];
...
void Greedy::determineMove(std::vector<int> &moveTo)
{
  for (int i = 0; i < Nnode; i++)
  {
    int oldM = i;
    ...
    if (newM != oldM)
    {
      ...
      // For all outLinks
      for (auto &[neighbor, flow] : node[i]->outLinks)
      {
        int nb_M = node[neighbor]->index;
        if (nb_M == oldM)
        {
          outFlowOldM += flow;
        }
      ...
      // For all inLinks
      for (auto &[neighbor, flow] : node[i]->inLinks)
      {
        int nb_M = node[neighbor]->index;
        if (nb_M == oldM)
        {
...
void Greedy::eigenvector()
{
  // cout << "Calculating steady state distribution of flow...";

  std::vector<double> size_tmp = std::vector<double>(Nnode, 1.0 / Nnode);
  int Niterations = 0;
  double danglingSize;
  double sqdiff = 1.0;
  do {
    ...
    const double sqdiff_old = sqdiff;
    sqdiff = 0.0;
    for (int i = 0; i < Nnode; i++)
    {
      node[i]->size /= sum;
