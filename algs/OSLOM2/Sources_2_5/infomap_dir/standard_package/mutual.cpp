...
double H_x_given_y(std::deque<std::deque<int>> &en, std::deque<std::deque<int>> &ten, int dim)
{
  ...
  for (auto& j : en)
  {
    ...
    for (auto& i : ten)
    {
      ...
      std::deque <int> s(dim);
      double I1_I2 = std::set_intersection(i.begin(), i.end(), j.begin(), j.end(), s.begin()) - s.begin();	// common
      double I1_02 = std::set_difference(i.begin(), i.end(), j.begin(), j.end(), s.begin()) - s.begin();
      double O1_I2 = std::set_difference(j.begin(), j.end(), i.begin(), i.end(), s.begin()) - s.begin();
