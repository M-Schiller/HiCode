...
#ifndef UNDIRECTED_OSLOMNET_EVALUATE_H
#define UNDIRECTED_OSLOMNET_EVALUATE_H
...
public:
  ...
  {
    set_graph(A);  set_maxbord(); set_changendi_cum();
  };
  ~oslomnet_evaluate() = default;
...
private:
  ...
  double all_external_test(int kout_g, int tm, int Nstar, int nneighs, const double & max_r_one, const double & maxr_two, std::deque<int> & gr_cleaned, bool only_c, weighted_tabdeg & previous_tab_c);
  ...
  double CUP_runs(weighted_tabdeg & previous_tab_c, weighted_tabdeg & previous_tab_n, int kin_cgroup_prev, int ktot_cgroup_prev, std::deque<int> & gr_cleaned, bool only_c, int runs);
  void initialize_for_evaluation(const std::deque<int> & _c_, weighted_tabdeg & previous_tab_c, weighted_tabdeg & previous_tab_n, int & kin_cgroup_prev, int & ktot_cgroup_prev);
  ...
  double partial_CUP(weighted_tabdeg & previous_tab_c, weighted_tabdeg & previous_tab_n, int kin_cgroup_prev, int ktot_cgroup_prev, std::deque<int> & gr_cleaned, bool only_c);
...
void oslomnet_evaluate::insert_cgroup(int wnode) {
  ...
  {
    std::map<int, facts>::iterator itm = neighs.lab_facts.find(wnode);
...
inline double oslomnet_evaluate::CUP_both(
  const std::deque<int> & _c_,
  std::deque<int> & gr_cleaned,
  int number_of_runs = paras.clean_up_runs)
{
  /*_c_ is the module to clean up and gr_cleaned is the result */
...
inline double oslomnet_evaluate::CUP_iterative(
  const std::deque<int> & _c_,
  std::deque<int> & gr_cleaned,
  int number_of_runs = paras.clean_up_runs)
{
...
inline double oslomnet_evaluate::group_inflation(const std::deque<int> & _c_, std::deque<int> & gr_cleaned, int number_of_runs = paras.inflate_runs)
{
  ...
  while (true) {
    ...
    if (!gr_cleaned.empty())
    {
      ...
      //cout<<"exiting... "<<gr_cleaned.size()<< std::endl;
      if (!gr_cleaned.empty())
      {
        return bscore;
      }
...
inline void oslomnet_evaluate::get_external_scores(
  ...
  weighted_tabdeg & previous_tab_c)
{
  auto bit = neighs.fitness_lab.begin();

  int counter = 0;

  while (bit != neighs.fitness_lab.end()) {
    auto itm = neighs.lab_facts.find(bit->second);
    double interval;
    ...
    if (F > max_r)
    {
      /*if(only_c == false || previous_tab_c.is_internal(itm->first))
        cout<<"no node: "<<vertices[itm->first]->id_num<<"  "<<itm->second.internal_degree<<" / "<< itm->second.degree<<" fitness: "<<F<< std::endl;*/
      ...
      /*if(only_c == false || previous_tab_c.is_internal(itm->first))
        cout<<"node: "<<"  "<<itm->second.internal_degree<<" / "<< itm->second.degree<<" fitness: "<<F<<" "<<interval<< std::endl;*/
...
#endif // UNDIRECTED_OSLOMNET_EVALUATE_H
