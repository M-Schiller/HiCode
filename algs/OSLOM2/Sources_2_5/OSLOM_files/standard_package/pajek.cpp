...
void set_partition_from_list(std::deque<int> & mems, std::deque<std::deque<int>> & ten)
{
  ten.clear();
  ...
  for (int& mem : mems)
  {
    value_com.insert(std::make_pair(mem, value_com.size()));
    id_value.insert(std::make_pair(a, mem));
    ++a;
  }
...
int get_partition_from_file_list_pajek_tree(std::string s, std::deque<std::deque<std::deque<int>>> & Ten)
{
  Ten.clear();
...
int pajek_format(std::string filename, bool directed)
{
...

  {
    std::ifstream inb(filename);
    std::string ins;
    while (std::getline(inb, ins)) if (!ins.empty() && ins[0] != '#')
    {
