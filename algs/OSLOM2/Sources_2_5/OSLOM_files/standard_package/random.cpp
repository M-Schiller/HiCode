...
void srand_file(void)
{
  ...
  srand5(seed);
  std::ofstream out("time_seed.dat");
  out << seed + 1 << std::endl;
}
...
int configuration_model(std::deque<std::set<int>> & en, std::deque<int> & degrees)
{
  ...
  if (degrees.size() < 3) {
    std::cerr << "it seems that some communities should have only 2 nodes! This does not make much sense (in my opinion) Please change some parameters!" << std::endl;
    return -1;
  }
  ...
  while (!degree_node.empty())
  {
    ...
    {
      if (i->first > 1)
        degree_node.insert(std::make_pair(i->first - 1, i->second));
      degree_node.erase(i);
    }
  ...
  for (int node_a = 0; node_a<int(degrees.size()); node_a++) for (int krm = 0; krm<int(en[node_a].size()); krm++)
  {
    ...
    if (en[node_a].insert(random_mate).second)
    {
      ...
      for (auto it_est = en[node_a].begin(); it_est != en[node_a].end(); ++it_est) if ((*it_est) != random_mate)
      {
      ...
      for (auto it_est = en[random_mate].begin(); it_est != en[random_mate].end(); ++it_est)
      {
