...
double mutual(std::deque<std::deque<int>> en, std::deque<std::deque<int>> ten) {
  ...
  for (int i = 0; i<int(ten.size()); i++)
    for (int j = 0; j<int(en.size()); j++)
      N[i][j] = std::set_intersection(ten[i].begin(), ten[i].end(), en[j].begin(), en[j].end(), s.begin()) - s.begin();
  ...
  cout<<"confusion matrix"<< std::endl;
  printm(N, std::cout);
  */
...
double H(std::deque <double> &p) {
  double h = 0;
  for (auto it = p.begin(); it != p.end(); it++) if (*it != 0)
    h += (*it)*log(*it);
  ...
  for (int j = 0; j<int(en.size()); j++) {
    ...
    for (int i = 0; i<int(ten.size()); i++) {
      ...
      if (H12_negative > H12_positive) {
        H12_ = H1_ + H2_;
        //cout<<"the negative part is bigger"<< std::endl;
        //prints(en[j]);
        //prints(ten[i]);
      }
      ...
      cout<<"worst case "<<H1_+H2_<<"\ttotal "<<H12_negative+H12_positive<<"\tnegative part "<<H12_negative<<"\tpositive part "<<H12_positive<< std::endl;
      prints(en[j]);
      prints(ten[i]);

      */

      //cout<<"entropies "<<H1_<<" "<<H2_<<" "<<H12_<< std::endl;
...
double H_x_given_y3(std::deque<std::deque<int>> &en, std::deque<std::deque<int>> &ten, int dim)
{
  ...
  {
    std::deque <double> p;
    const auto I2 = double(c.size());
    const double O2 = (dim - I2);
    p.push_back(I2 / dim);
    p.push_back(O2 / dim);
    const double H2_ = H(p);
    p.clear();
    ...
    for (int i : c)
    {
      for (int j : mems[i])
      {
        int_histogram(j, com_ol);
      }
    }

    for (auto& itm : com_ol)
    {
      const double I1 = double(ten[itm.first].size());
      const double O1 = (dim - I1);

      p.push_back(I1 / dim);
      p.push_back(O1 / dim);
      const double H1_ = H(p);
      p.clear();

      const double I1_I2 = itm.second;
      const double I1_02 = ten[itm.first].size() - I1_I2;
      const double O1_I2 = c.size() - I1_I2;
      const double O1_02 = dim - I1_I2 - I1_02 - O1_I2;

      p.push_back(I1_I2 / dim);
      p.push_back(O1_02 / dim);

      const double H12_positive = H(p);

      p.clear();
      p.push_back(I1_02 / dim);
      p.push_back(O1_I2 / dim);

      const double H12_negative = H(p);
