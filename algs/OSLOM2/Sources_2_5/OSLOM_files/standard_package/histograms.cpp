...
template <typename T>
int xybinning(std::deque<T> &c, std::deque<T> &d, int number_of_bins, std::deque<double> & xs, std::deque<double> & ys, std::deque<double> & var, std::deque<int> & nums)
{
  ...
  double qb)
{
  ...
  for (int i = 0; i < hist_x.size() - 1; i++)
  {
    ...
    if (!hist_y[i].empty())
    {
      ...
      var.push_back(variance_func(hist_y[i]) / double(hist_y[i].size()));
      nums.push_back(hist_y[i].size());
      std::sort(hist_y[i].begin(), hist_y[i].end());
...
int log_histogram(std::deque<T> &c, std::ostream & out, int number_of_bins)		// c is the set od data, min is the lower bound, max is the upper one
{
  ...
  for (int i = 0; i<int(c.size()); i++)
  {
    ...
    {
      if ((fabs(double(c[i]) - bins[j]) < 1e-7) || (double(c[i]) > bins[j] && double(c[i]) < bins[j + 1]))
      {
...
int log_histogram(std::deque<double> &c, std::deque<double> &c2, std::ostream & out, int number_of_bins)
{
...
void int_histogram(std::vector <int> &c, std::ostream & out)
{
  ...
  for (int i = 0; i<int(c.size()); i++)
  {
    const std::map<int, double>::iterator itf = hist.find(c[i]);
    if (itf == hist.end())
    {
      hist.emplace(c[i], 1.);
    }
...
int print_cumulative(std::deque<double> & kws, std::string file, int number_of_step)
{
...
int print_cumulative(std::deque<int> & kws, std::string file, int number_of_step)
{
  std::ofstream expout(file);
  sort(kws.begin(), kws.end());
...
int print_cumulative(std::vector<double> & kws, std::string file, int number_of_step)
{
  std::ofstream expout(file);
  sort(kws.begin(), kws.end());
...
int print_cumulative(std::vector<int> & kws, std::string file, int number_of_step)
{
  std::ofstream expout(file);
  sort(kws.begin(), kws.end());
...
void int_histogram(std::string infile, std::string outfile)
{
...
void int_histogram(const int & c, std::map<int, std::pair<int, double>>  & hist, const int & w1, const double  & w2)
{
  auto itf = hist.find(c);
  if (itf == hist.end())
  {
    hist.emplace(c, std::make_pair(w1, w2));
  }
  else
  {
    itf->second.first += w1;
