...
#ifndef LOUVAIN_OSLOMNET_H
#define LOUVAIN_OSLOMNET_H
...
void prints(map_int_om & M)
{
  for (auto itm = M.begin(); itm != M.end(); ++itm)
    std::cout << "module: " << itm->first << "\t\t\t\tnc= " << itm->second.nc << "\t ktot= " << itm->second.ktot << "\t kout= " << itm->second.kout << std::endl;
}
...
inline void oslomnet_louvain::unweighted_favorite_of(const int & node, int & fi, int & kp, int & kop) {
  ...
  for (std::map<int, int>::iterator itM = M.begin(); itM != M.end(); ++itM) {
...
inline void oslomnet_louvain::weighted_favorite_of(const int & node, int & fi, int & kp, int & kop)
{
  double min_fitness = 10;
  fi = vertex_label[node];
  kp = 0;
  ...
  for (auto itM = M.begin(); itM != M.end(); ++itM) {
    ...
    if (itM->first != vertex_label[node]) {
      ...
      to_fit *= double(dim - itOM->second.nc + 1) / (std::min(dim - itOM->second.nc, itOM->second.kout / itM->second.first + 1) + 1);
      ...
      to_fit *= double(dim - itOM->second.nc + 2) / (std::min(dim - itOM->second.nc + 1, kout_prime / kop + 1) + 1);
...
inline void oslomnet_louvain::single_pass_unweighted()
{
  ...
  for (std::deque<int> ::iterator itd = vertex_order.begin(); itd != vertex_order.end(); ++itd) {
...
int oslomnet_louvain::check_all() {
  ...
  for(int i=0; i<int(M.size()); i++) {
    ...
    int _kin= kin_m(M[i]);
    if(print_stuff_oslom_local)
      cout<<"nc: "<<M[i].size()<<" kout: "<<_ktot - _kin<<" ktot: "<<_ktot<< std::endl;
