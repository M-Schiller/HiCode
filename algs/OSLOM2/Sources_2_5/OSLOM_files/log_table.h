...
public:
  ...
  {
    return std::max(0., exp(log_hyper(kin_node, kout_g, tm, degree_node)));
  }
...
private:
  ...
  double sym_ratio(int & k1, int & k2, int & H, double i) { return 0.5 * (k1 - i + 1) / ((i + H)*i)   * (k2 - i + 1); };
...
inline void log_fact_table::_set_(int size)
{
...
inline double log_fact_table::cum_hyper_right(int kin_node, int kout_g, int tm, int degree_node)
{
  //cout<<"kin_node... "<<kin_node<<" "<<kout_g<<" "<<tm<<" "<<degree_node<< std::endl;
  ...
  int x = kin_node;
  const double pzero = hyper(x, kout_g, tm, degree_node);
  ...
  const double ga = tm - kout_g - degree_node;
  const int kout_g_p = kout_g + 1;
  const double degree_node_p = degree_node + 1;
  double z_zero = 1.;
  double sum = z_zero;
...
inline double log_fact_table::cum_hyper_left(int kin_node, int kout_g, int tm, int degree_node)
{
  // this is strictly less  p(x < kin_node)   *** NOT EQUAL ***
  //cout<<kin_node<<" node: "<<degree_node<<" group: "<<tm<<" "<<degree_node<< std::endl;
  ...
  //cout<<"pzero: "<<pzero<<" "<<log_hyper(x, kout_g, tm, degree_node)<<" gsl: "<<(gsl_ran_hypergeometric_pdf(x, kout_g, tm - kout_g,  degree_node))<< std::endl;
  ...
  //cout<<"pzero "<<pzero<<" "<<z_zero<<" "<<kin_node<< std::endl;

  while (true)
  {
    z_zero *= (ga + x) / ((degree_node_p - x) *(kout_g_p - x))  * x;
    --x;
    //cout<<"zzero sum "<<z_zero<<" "<<sum<<" "<<(ga + x)<< std::endl;
...
inline double log_fact_table::cum_binomial_right(int x, int N, double prob)
{
  // this is bigger  or equal p(x >= kin_node)   *** EQUAL ***

  //cout<<"x "<<x<<" N "<<N <<"  prob "<<prob<< std::endl;
  ...
  const double pzero = binom(x, N, prob);
  ...
  while (true)
  {
    ...
    x++;
    //cout<<"zzero sum "<<z_zero<<" "<<sum<<" "<< std::endl;
...
inline double log_fact_table::cum_binomial_left(int x, int N, double prob)
{
...
inline double log_fact_table::slow_symmetric_eq(int k1, int k2, int H, int x)
{
  ...
  {
    //cout<<ix<<" "<<(log_symmetric_eq(k1, k2, H, ix))<< std::endl;
...
inline double log_fact_table::fast_right_cum_symmetric_eq(int k1, int k2, int H, int x, int mode, int tm)
{
  ...
  //cout<<"k1 "<<k1<<" "<<k2<<" "<<H<<" "<<x<<" "<<mode<<" "<<2*H+k1+k2<< std::endl;
  ...
  for (auto ii = mode - 1; ii >= std::max(0, -H); --ii)
  {
  ...
  for (auto ii = mode + 1; ii < x; ++ii)
  {
    ...
    if (ri < log_table_pr * q1)
      break;
    //cout<<ii<<" "<<ratio<<" "<<ri/q1<<" b"<< std::endl;
    //cout<<"dx-->: "<<ii<<" "<<exp(log_symmetric_eq(k1, k2, H, ii))<<" "<<exp(lg0) * ri<<" "<<sym_ratio(k1, k2, H, ii+1)<< std::endl;
  }

  auto ii = std::max(x, mode + 1);
  ...
  while (ii <= k1)
  {
    ...
    if (q2 > 1e280)
      return cum_hyper_right(x, k2, tm, k1);
    //cout<<ii<<" "<<ratio<<" "<<ri/q2<<" c "<<x<<" "<<q2<< std::endl;;
    ++ii;
    if (ri < log_table_pr * q2)
      break;

    //cout<<"ddx-->: "<<i<<" "<<exp(log_symmetric_eq(k1, k2, H, i))<<" "<<exp(lg0) * ri<<" "<<sym_ratio(k1, k2, H, i+1)<< std::endl;
  }
