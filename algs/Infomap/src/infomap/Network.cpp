...
void Network::parsePajekNetwork(const std::string& filename)
{
  ...
  else
  {
    ...
    for (unsigned int i = 0; i < numNodes; ++i)
    {
      ...
      //		m_nodeMap.insert(std::make_pair(name, i));
      m_nodeNames[i] = name;
    }
  ...
  while (!std::getline(input, line).fail())
  {
    ...
    auto it = m_links.find(std::make_pair(linkEnd1, linkEnd2));
    if (it == m_links.end())
    {
...
void Network::parseLinkList(const std::string& filename)
{
  ...
  {
    //		m_nodeMap.insert(std::make_pair(io::stringify(i + 1), i));
    m_nodeNames[i] = io::stringify(i + 1);
  }
...
void Network::parseSparseLinkList(const std::string& filename)
{
  std::string line;
  std::string buf;
  SafeInFile input(filename.c_str());
  ...
  while (!std::getline(input, line).fail())
  {
    ...
    // Aggregate link weights if they are definied more than once
    auto fromLinkIt = links.find(linkEnd1);
    ...
    else
    {
      ...
      {
        fromLinkIt->second.insert(std::make_pair(linkEnd2, linkWeight));
        ++numLinks;
      }
      else {
  ...
  for (auto linkIt(links.begin()); linkIt != links.end(); ++linkIt, ++linkIndex)
  {
    packedIndices.insert(std::make_pair(linkIt->first, linkIndex));
    m_nodeMap.insert(std::make_pair(io::stringify(linkIt->first + 1), linkIndex));
  ...
  for (OutLinkMap::iterator linkIt(links.begin()); linkIt != links.end(); ++linkIt, ++linkIndex)
  {
      ...
      toLinkIt != outLinkMap.end(); ++toLinkIt)
    {
      ...
      m_links.insert(std::make_pair(std::make_pair(first, second), toLinkIt->second));
    }
  }
...
void Network::parsePajekNetworkCStyle(const std::string& filename)
{
  RELEASE_OUT("Parsing " << (m_config.isUndirected() ? "undirected" : "directed") << " network from file '" <<
...
bool Network::parseEdgeCStyle(char line[], unsigned int& sourceIndex, unsigned int& targetIndex, double& weight)
{
  ...
  if (cptr == NULL)
    return false;
  sourceIndex = std::atoi(cptr); // get first connected node
  cptr = strtok(NULL, " \t"); // Get second non-whitespace character position
  if (cptr == NULL)
    return false;
  targetIndex = std::atoi(cptr); // get the second connected node
  cptr = strtok(NULL, " \t"); // Get third non-whitespace character position
  if (cptr != NULL)
    weight = std::atof(cptr); // get the link weight
  else
    weight = 1.0;
...
void Network::parseLinkListCStyle(const std::string& filename)
{
  ...
  while (fgets(line, LINELENGTH, file) != NULL)
  {
    ...
    if (checkNodeLimit
      && (linkEnd2 >= nodeLimit || linkEnd1 >= nodeLimit))
    {
      ++numSkippedEdges;
      continue;
    }
...
void Network::printNetworkAsPajek(const std::string& filename)
{
