...
void FlowNetwork::calculateFlow(const Network& network, const Config& config)
{
  ...
  for (auto linkIt(linkMap.begin()); linkIt != linkMap.end(); ++linkIt, ++linkIndex)
  {
  ...
  if (config.rawdir)
  {
    ...
    m_nodeFlow.assign(numNodes, 0.0);
    for (auto linkIt(m_flowLinks.begin()); linkIt != m_flowLinks.end(); ++linkIt)
    {
      Link& link = *linkIt;
  ...
  do
  {
    ...
    danglingRank = 0.0;
    for (auto danglingIt(danglings.begin()); danglingIt != danglings.end(); ++danglingIt)
    {
