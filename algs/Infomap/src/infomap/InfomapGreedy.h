    ...
    nodeIt != nodeEnd; ++nodeIt)
  {
      ...
      edgeIt != edgeEnd; ++edgeIt)
    {
      ...
      if (otherParent != parent)
      {
        ...
        std::pair<EdgeMap::iterator, bool> ret = moduleLinks.emplace(NodePair(m1, m2), edge->data.flow);
        if (!ret.second)
        {
...
{
  typedef std::multimap<double, EdgeType*, std::greater<>> LinkMap;
  typedef std::multimap<double, NodeBase*, std::greater<>> NodeMap;
    ...
    moduleIt != endIt; ++moduleIt, ++moduleIndex)
  {
    ...
    while (li != liEnd)
    {
      moduleData[moduleIndex].second.emplace(getNode(*li).data.flow, li.base());
      ++li;
    }

    // Collect module links
    for (auto outEdgeIt(moduleIt->begin_outEdge()), endIt(moduleIt->end_outEdge()); outEdgeIt != endIt; ++outEdgeIt)
    {
      moduleLinks.emplace((*outEdgeIt)->data.flow, *outEdgeIt);
    }
  }
