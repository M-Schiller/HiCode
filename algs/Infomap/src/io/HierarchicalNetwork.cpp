...
void HierarchicalNetwork::writeMap(const std::string& fileName)
{
  ...
  for (unsigned int i = 0; i < numModules; ++i)
  {
    ...
    while (li != liEnd)
    {
      ...
      nodeMap.emplace(li->data.flow, li.base());
      ++li;
      ++numNodes;
    }
  }
  ...
  for (unsigned int i = 0; i < numModules; ++i)
  {
    ...
    for (auto it(nodeMap.begin()), itEnd(nodeMap.end());
  ...
  for (auto it(m_rootNode.childEdges.begin()); it != m_rootNode.childEdges.end(); ++it)
  {
