...
public:
  // Capacity:
  unsigned int serializationSize(bool writeEdges) const
  {
    ...
    if (writeEdges && parentNode != nullptr
      && static_cast<unsigned int>(parentIndex + 1) == parentNode->children.size())
    {
      // numEdges + {edges}
  ...
  // Operations:
  void serialize(SafeBinaryOutFile& outFile, SerialTypes::childPos_t childPosition, bool writeEdges) const
  {
    using namespace SerialTypes;
    ...
    auto numChildren = numeric_cast<childSize_t>(children.size());
    ...
    if (writeEdges && parentNode != NULL && static_cast<unsigned int>(parentIndex + 1) == parentNode->children.size())
    {
      ...
      auto numEdges = numeric_cast<edgeSize_t>(edges.size());
      // First sort the edges
      std::multimap<double, ChildEdge, std::greater<>> sortedEdges;
      for (auto edge : edges)
      {
        sortedEdges.emplace(edge.flow, edge);
      }
      outFile << numEdges;
      std::multimap<double, ChildEdge, std::greater<>>::const_iterator it(sortedEdges.begin());
  ...
  SNode* nextSibling() const
  {
    if (parentNode == nullptr || static_cast<unsigned int>(parentIndex + 1) == parentNode->children.size())
      return nullptr;
...
public:
    ...
    m_depth(0)
  {
    ...
    {
      while (m_current->firstChild() != nullptr)
      {
  ...
  LeafIterator&
    operator++()
  {
    while (m_current->nextSibling() == nullptr)
    {
...
public:
  ...
  virtual ~HierarchicalNetwork() = default;
  ...
  SNode& addLeafNode(SNode& parent, double flow, double exitFlow, std::string name, unsigned int leafIndex)
  {
    ...
    while (node != nullptr && node->depthBelow < currentDepthBelow)
    {
  ...
  void writeStreamableTree(const std::string& fileName, bool writeEdges)
  {
    ...
    using namespace SerialTypes;
    while (!nodeList.empty()) // visit tree breadth first
    {
      ...
      auto numChildren = numeric_cast<childSize_t>(node.children.size());
